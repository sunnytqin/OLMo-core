...
def detect_model_size(checkpoint_path: Path) -> str:
    """Detect model size from checkpoint path (e.g. '370M' from '370M_seed42_...')."""
    for part in [checkpoint_path.parent.name, checkpoint_path.name]:
        for size in MODEL_CONFIG_MAP:
            if part.startswith(size) or f"/{size}_" in f"/{part}":
                return size
    raise ValueError(f"Cannot detect model size from {checkpoint_path}. "
                     f"Expected one of {list(MODEL_CONFIG_MAP.keys())} in path.")
...
def convert_checkpoint(checkpoint_path: Path, output_dir: Path) -> Path:
    ...
    config_fn = getattr(TransformerConfig, MODEL_CONFIG_MAP[model_size])
