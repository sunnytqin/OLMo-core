...
echo "Job $SLURM_JOB_ID  cutoff=${N_MAX_MIL}M  anchored=$ANCHORED"
echo "Out: $OUT_JSON"
echo "Started: $(date)"

CMD=(/home/hamidieh/anaconda3/bin/python fit_triple_extrapolate.py
     --n-max-mil "$N_MAX_MIL"
     --out-json "$OUT_JSON")
[ "$ANCHORED" = "yes" ] && CMD+=(--anchored) || CMD+=(--no-anchored)
